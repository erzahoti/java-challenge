	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < len; i++) {
			for (int j = 0; j < len; j++) {
				...
				// horizontal
				if ((j + 3) <len) {
					maxProduct = findProduct(matrix[i][j], matrix[i][j + 1], matrix[i][j + 2], matrix[i][j + 3], maxProduct);
				}
		...
		System.out.println("The highest possible product of 4 numbers in text string: "+maxProduct);
	
	}

	static int[][] generateMatrix(String txt) {
		String[] splited = txt.split("\\n+");
		int len = splited.length;
		int[][] matrix = new int[len][len];

		for (int i = 0; i < len; i++) {
			String[] row = splited[i].split("\\s");
